# MBZ GPU Setup 
1. Make a new env and follow the installation procedure given [here](fastchat/llm_judge/README.md). Make sure the HF installation has the necessary changes for your models. 
> If you are using the env specified in the slurm script, please do not change those envs. Used them as is. Any new installation may break the env. Those are used for other critical work as well. 
2. See the [slurm_script](fastchat/llm_judge/submit_slurm.sh). Fill in the following details: 
    - model_ids and paths
    - GPU setup, how many GPUs for how many models. For heavier model, increase GPUs per Model. 
    - Set the OPENAI_API_KEY 
3. Run the slurm script. If its a 7-9B model, it will take around 5 mins to generate the answers and 30 mins to judge the answers. You will see the output in the .out and .err files generated by slurm. 
